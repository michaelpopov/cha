...
it('reloads after a successful vault switch', async () => {
  const user = userEvent.setup();
...
it('keeps the current vault selected after a recoverable switch failure', async () => {
  const user = userEvent.setup();
  const switchVault = vi.fn(async () => {
    throw new ChaError(500, 'internal_error', 'Could not pause active sessions.');
  });
  const reload = vi.fn();
  render(
    <App
  ...
  await screen.findByLabelText('Vault');
  await user.selectOptions(screen.getByLabelText('Vault'), 'Projects');
  expect(await screen.findByRole('alert')).toHaveTextContent('Could not pause active sessions.');
  expect(screen.getByLabelText('Vault')).toHaveValue('Personal');
  expect(screen.getByLabelText('Vault')).toBeEnabled();
  expect(reload).not.toHaveBeenCalled();
});

it('disables the vault selector while a switch is pending', async () => {
  const user = userEvent.setup();
  const held = deferred();
  const switchVault = vi.fn(() => held.promise);
  render(
    <App
      client={fixtureClient({ switchVault })}
      connectSessionEvents={inertSessionEvents}
  ...
  await screen.findByLabelText('Vault');
  await user.selectOptions(screen.getByLabelText('Vault'), 'Projects');
  await waitFor(() => expect(screen.getByLabelText('Vault')).toBeDisabled());
  held.settle();
});

it('keeps matching snapshots on the normal open and stream path', async () => {
  const events = drivableSessionEvents();
  const reload = vi.fn();
  render(
    <App
...
it('reloads on a mismatched open snapshot and does not attach its stream', async () => {
  const events = drivableSessionEvents();
  const reload = vi.fn();
  const getSessionSnapshot = vi.fn(async (forumId: string) => (
    forumId === 'lobby'
      ? { ...lobbySnapshot(), vault_name: 'Projects' }
      : snapshotFixture
  ));
  render(
    <App
      client={storedPlanningClient({ getSessionSnapshot })}
      connectSessionEvents={events.connect}
      reload={reload}
    />,
  );
  ...
  expect(events.connections.filter(({ key }) => key === 'lobby/planning')).toHaveLength(0);
  expect(window.location.pathname).toBe('/');
});

it('reloads on a mismatched SSE snapshot of entrance/welcome and never publishes it', async () => {
  const events = drivableSessionEvents();
  const reload = vi.fn();
  render(
    <App
  ...
  await waitFor(() => expect(reload).toHaveBeenCalled());
  expect(screen.queryByLabelText('Current chat context')).not.toHaveTextContent('Projects');
});

it('reloads when a bootstrap refresh names another vault', async () => {
  const reload = vi.fn();
  const otherVault = {
    ...bootstrapFixture,
    vault_name: 'Projects',
  ...
  const getBootstrap = vi.fn()
    .mockResolvedValueOnce(bootstrapFixture)
    .mockResolvedValue(otherVault);
  render(
    <App
      client={storedPlanningClient({ getBootstrap })}
      connectSessionEvents={inertSessionEvents}
      reload={reload}
    />,
  );
...
it('keeps a same-vault bootstrap refresh without reloading', async () => {
  const reload = vi.fn();
  const refreshed = structuredClone(bootstrapFixture);
  refreshed.recent_sessions = [
    {
  ...
  await openPlanningFromTheLobby();
  await waitFor(() => expect(screen.getByRole('button', { name: /^Created/ })).toBeInTheDocument());
  expect(reload).not.toHaveBeenCalled();
  expect(screen.getByLabelText('Vault')).toHaveValue('Personal');
});

it('reloads when recovery probes a snapshot from another vault', async () => {
  const events = drivableSessionEvents();
  const reload = vi.fn();
  const getSessionSnapshot = vi.fn()
    .mockResolvedValueOnce(lobbySnapshot())
    .mockResolvedValue({ ...lobbySnapshot(), vault_name: 'Projects' });
  render(
    <App
      client={storedPlanningClient({ getSessionSnapshot })}
      connectSessionEvents={events.connect}
  ...
  const planning = events.connections.findIndex(({ key }) => key === 'lobby/planning');
  act(() => events.handlers[planning].onSnapshot(lobbySnapshot()));
  const attached = events.connections.filter(({ key }) => key === 'lobby/planning').length;
  act(() => events.handlers[planning].onError({ kind: 'stream_failure' }));
  await waitFor(() => expect(reload).toHaveBeenCalled());
  expect(events.connections.filter(({ key }) => key === 'lobby/planning')).toHaveLength(attached);
  expect(screen.queryByLabelText('Current chat context')).not.toHaveTextContent('Projects');
});

it('reloads once when recovery probes a snapshot from another vault', async () => {
  const events = drivableSessionEvents();
  const reload = vi.fn();
  const getSessionSnapshot = vi.fn()
    .mockResolvedValueOnce(lobbySnapshot())
    .mockResolvedValueOnce({ ...lobbySnapshot(), vault_name: 'Projects' })
    .mockResolvedValue(lobbySnapshot());
  render(
    <App
      client={storedPlanningClient({ getSessionSnapshot })}
      connectSessionEvents={events.connect}
  ...
  await openPlanningFromTheLobby();
  const planning = () => events.connections.findIndex(({ key }) => key === 'lobby/planning');
  await waitFor(() => expect(planning()).toBeGreaterThanOrEqual(0));
  act(() => events.handlers[planning()].onSnapshot(lobbySnapshot()));
  act(() => events.handlers[planning()].onError({ kind: 'stream_failure' }));
  await waitFor(() => expect(reload).toHaveBeenCalledTimes(1));
});

it('ends recovery of a missing old session in the existing retry state', async () => {
  const events = drivableSessionEvents();
  const reload = vi.fn();
  const missing = new ChaError(409, 'session_not_live', 'That session is not open.');
  const gone = new ChaError(404, 'not_found', 'That forum or session was not found.');
  const getSessionSnapshot = vi.fn()
    .mockResolvedValueOnce(lobbySnapshot())
    .mockRejectedValue(missing);
  const openSession = vi.fn(async (forumId: string, sessionId: string) => {
    if (sessionId === 'planning' && getSessionSnapshot.mock.calls.length > 1) {
      throw gone;
    }
    return { forum_id: forumId, session_id: sessionId };
  });
  render(
    <App
      client={storedPlanningClient({ getSessionSnapshot, openSession })}
      connectSessionEvents={events.connect}
      reload={reload}
      retryDelays={[0, 0]}
    />,
  ...
  await waitFor(() => expect(screen.getByRole('alert'))
    .toHaveTextContent('Live updates could not be restored.'));
  expect(reload).not.toHaveBeenCalled();
});
