...
export function fixtureClient(overrides: Partial<ChaClient> = {}): ChaClient {
  ...
  const client: ChaClient = {
    ...
    listVaults: async () => [],
    createVault: async () => { throw new Error('No vault fixture'); },
    listR2Vaults: async () => [],
    downloadR2Vault: async () => { throw new Error('No R2 vault fixture'); },
    updateVault: async () => { throw new Error('No vault fixture'); },
    ...
    startXaiVoiceInput: async (sessionId) => ({ session_id: sessionId, stop_budget_ms: 20000 }),
    sendXaiVoiceAudio: async (sessionId) => ({ session_id: sessionId, pieces: [] }),
    stopXaiVoiceInput: async (sessionId) => ({ session_id: sessionId, pieces: [] }),
