...
// halves: read-only session state (conversation, roster, default agent, generation status) and
// commands (submit a prompt, clear, stop, switch the default agent, drain agent events),
// each returning a SessionUpdate instead of touching the UI. It composes Conversation,
...
public:
    ...
    [[nodiscard]] SessionUpdate submit_prompt(
        std::string text,
        std::string handle = {});
    [[nodiscard]] SessionUpdate clear_conversation();
