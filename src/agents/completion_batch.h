...
// One in-flight completion operation. It owns the ordered execution slots
...
// still reach a backend. Each slot owns the one CompletionInput its backend
...
public:
    ...
    CompletionBatch(const CompletionBatch&) = delete;
    CompletionBatch& operator=(const CompletionBatch&) = delete;
...
private:
    ...
    [[nodiscard]] static CompletionBatch stage(
    ...
    explicit CompletionBatch(std::unique_ptr<Impl> impl) noexcept;
