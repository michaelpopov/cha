...
`interfaces/terminal/` is the ncurses front end: it owns the terminal itself,
...
Everything that decides *what* a session does lives in `application/`. This
...
| `startup_selector.*` | The room and session pickers, plus the new-session name prompt, drawn from application-safe values. |
...
| `user_session.*` | The session state machine: input to actions, `CoordinatorUpdate` to screen. |
...
flowchart TD
    ...
    agent -->|"yes"| drain["coordinator.receive,<br/>apply updates"]
    agent -->|"no"| keys
...
the resulting `CoordinatorUpdate` is applied uniformly — clear the editor, set
...
line, and a boxed multiline input pad. Below a minimum size the interface draws
...
- **Depends on:** `application/` for coordinator operations, generation status,
  and session summaries; `conversation/` for snapshots and entries;
  `interfaces/text/` for command dispatch; `agents/` for roster values used in
...
| Test | Covers |
| --- | --- |
| `tests/interfaces/terminal/unit_user_session.cpp` | The state machine against a fake `SessionView`: input handling, update application, cancel-versus-exit, notices. |
| `tests/interfaces/terminal/unit_transcript_renderer.cpp` | Render plans, labels and styling through a recording `TranscriptSurface`, viewport scrolling, row layout. |
| `tests/interfaces/terminal/unit_user.cpp` | Event-loop readiness decoding and shutdown ordering. |
