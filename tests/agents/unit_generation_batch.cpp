...
std::vector<std::unique_ptr<ModelBackend>> two_backends(
    std::unique_ptr<ModelBackend> first,
    std::unique_ptr<ModelBackend> second) {
...
TEST(GenerationBatch, ClosedGateHoldsEveryExecutionUntilOpened) {
    ...
    batch.open();
    EXPECT_TRUE(std::holds_alternative<GenerationEventDelta>(next_foreground_event(batch)));
    EXPECT_TRUE(
...
TEST(GenerationBatch, CancellationBeforeOpeningSkipsPreparationAndPerformance) {
    Transcript transcript;
    ThreadPool pool(1);
    auto backend = std::make_unique<RecordingBackend>("one-id", "One", "one");
    RecordingBackend* const view = backend.get();
    ...
    EXPECT_EQ(
        std::get<GenerationCancelled>(next_foreground_event(batch)).request_id, 1U);
    EXPECT_FALSE(view->prepared.load(std::memory_order_acquire));
    EXPECT_FALSE(view->performed.load(std::memory_order_acquire));
}

TEST(GenerationBatch, CancellationAfterOpeningStillSkipsAQueuedExecution) {
    Transcript transcript;
    ThreadPool pool(1);
    ...
    GenerationExecutor executor(
        test::one_backend(std::move(backend)), notifier(), pool);
    GenerationBatch batch = executor.stage_batch(
        {generation_request(transcript, 14, "one-id", "One")});
    batch.open();
    batch.cancel();
    release_blocker.store(true, std::memory_order_release);

    EXPECT_EQ(
        std::get<GenerationCancelled>(next_foreground_event(batch)).request_id, 14U);
    EXPECT_FALSE(view->prepared.load(std::memory_order_acquire));
    EXPECT_FALSE(view->performed.load(std::memory_order_acquire));
}

TEST(GenerationBatch, OpeningStartsEverySelectedBackendAtFullPoolWidth) {
    Transcript transcript;
    BarrierState state;
    ThreadPool pool(3);
    std::vector<std::unique_ptr<ModelBackend>> backends;
    ...
    GenerationBatch batch = executor.stage_batch({
        ...
        generation_request(transcript, 3, "three-id", "Three"),
    });
    batch.open();
...
TEST(GenerationBatch, ForegroundRunAndEventsComeFromTheSameSlot) {
    Transcript transcript;
    ThreadPool pool(2);
    ...
    GenerationBatch batch = executor.stage_batch({
        generation_request(transcript, 1, "one-id", "One", "One question"),
        generation_request(transcript, 2, "two-id", "Two", "Two question"),
    });
    ...
    EXPECT_EQ(std::get<GenerationEventDelta>(next_foreground_event(batch)).text, "first");
    EXPECT_EQ(
        std::get<GenerationCompleted>(next_foreground_event(batch)).request_id, 1U);
    ...
    EXPECT_EQ(std::get<GenerationEventDelta>(next_foreground_event(batch)).text, "second");
    EXPECT_EQ(
        std::get<GenerationCompleted>(next_foreground_event(batch)).request_id, 2U);

    EXPECT_EQ(one_view->received.run.prompt_text, "One question");
    EXPECT_EQ(two_view->received.run.prompt_text, "Two question");
}

TEST(GenerationBatch, LaterSlotsBufferEventsUntilForegroundAdvancement) {
    Transcript transcript;
...
TEST(GenerationBatch, AdvancementRequiresAConsumedTerminalAndANextSlot) {
    Transcript transcript;
    ThreadPool pool(2);
    GenerationExecutor executor(
        two_backends(
...
TEST(GenerationBatch, PreservesCapturedHistoryForEveryExecution) {
    Transcript transcript;
...
TEST(GenerationBatch, MapsGenerationFailureAndExceptionsToGenerationFailed) {
    Transcript transcript;
    ThreadPool failure_pool(1);
    GenerationExecutor failure_executor(
        test::one_backend(std::make_unique<RecordingBackend>(
            "one-id",
            "One",
            "",
            GenerationResult{
                GenerationOutcome::protocol_error, "malformed response"})),
        notifier(),
        failure_pool);
    GenerationBatch failure_batch = failure_executor.stage_batch(
        {generation_request(transcript, 12, "one-id", "One")});
    failure_batch.open();
    const GenerationFailed failed =
        std::get<GenerationFailed>(next_foreground_event(failure_batch));
    EXPECT_EQ(failed.request_id, 12U);
    EXPECT_EQ(failed.message, "malformed response");

    ThreadPool throwing_pool(1);
    GenerationExecutor throwing_executor(
        test::one_backend(std::make_unique<ThrowingBackend>()),
        notifier(),
        throwing_pool);
    GenerationBatch throwing_batch = throwing_executor.stage_batch(
        {generation_request(transcript, 1, "one-id", "One")});
    throwing_batch.open();
    const GenerationFailed thrown =
        std::get<GenerationFailed>(next_foreground_event(throwing_batch));
    EXPECT_EQ(thrown.request_id, 1U);
    EXPECT_NE(thrown.message.find("preparation failed"), std::string::npos);
}

TEST(GenerationBatch, DeliversExactlyOneTerminalThenReportsClosed) {
    Transcript transcript;
    ThreadPool pool(1);
    GenerationExecutor executor(
        test::one_backend(std::make_unique<RecordingBackend>(
...
TEST(GenerationBatch, CancellationReachesEveryOpenedExecution) {
    Transcript transcript;
    BarrierState state;
    ThreadPool pool(2);
    GenerationExecutor executor(
    ...
    EXPECT_TRUE(
        std::holds_alternative<GenerationCancelled>(next_foreground_event(batch)));
    batch.advance_foreground();
    EXPECT_TRUE(
        std::holds_alternative<GenerationCancelled>(next_foreground_event(batch)));
}

TEST(GenerationBatch, ExplicitWaitingLeavesTerminalEventsDrainable) {
    Transcript transcript;
    BarrierState state;
    ThreadPool pool(1);
    GenerationExecutor executor(
...
TEST(GenerationBatch, DestructionCancelsAndWaitsForEveryExecution) {
    Transcript transcript;
    BarrierState state;
    ThreadPool pool(2);
    GenerationExecutor executor(
    ...
    GenerationBatch later = executor.stage_batch({
    ...
    EXPECT_TRUE(
        std::holds_alternative<GenerationCompleted>(next_foreground_event(later)));
}

TEST(GenerationBatch, CompletedAndCancelledBatchesPermitLaterBatches) {
    Transcript transcript;
    BarrierState state;
    ThreadPool pool(1);
    GenerationExecutor executor(
    ...
    {
        GenerationBatch completed = executor.stage_batch(
            {generation_request(transcript, 1, "one-id", "One")});
        completed.open();
        ASSERT_TRUE(wait_until_entered(state, 1));
        state.release.store(true, std::memory_order_release);
    ...
    state.release.store(false, std::memory_order_release);
    {
        GenerationBatch cancelled = executor.stage_batch(
    ...
    state.release.store(true, std::memory_order_release);
    GenerationBatch third = executor.stage_batch(
        {generation_request(transcript, 3, "one-id", "One")});
    third.open();
    EXPECT_TRUE(
        std::holds_alternative<GenerationCompleted>(next_foreground_event(third)));
}

TEST(GenerationBatch, MoveTransfersOwnershipWithoutDisturbingExecutions) {
    Transcript transcript;
    ThreadPool pool(1);
    GenerationExecutor executor(
        test::one_backend(std::make_unique<RecordingBackend>(
    ...
    std::optional<GenerationBatch> owner;
    owner.emplace(executor.stage_batch(
        {generation_request(transcript, 7, "one-id", "One")}));
    EXPECT_EQ(owner->foreground_run().request_id, 7U);
    owner->open();

    EXPECT_EQ(std::get<GenerationEventDelta>(next_foreground_event(*owner)).text, "answer");
    EXPECT_TRUE(
        std::holds_alternative<GenerationCompleted>(next_foreground_event(*owner)));
    owner.reset();
}
