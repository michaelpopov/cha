...
TEST(SessionRoutes, ServesLivePageSnapshotAndOwnerQueuedCommands) {
    ...
    const auto input = server.client().Post(base + "/api/v1/input", R"({"user":"reader","text":"/@Guide"})", "application/json");
    ASSERT_TRUE(input);
    EXPECT_EQ(input->status, 200);
    EXPECT_EQ(json_body(input), nlohmann::json({{"clear_input", true}, {"notice", "input accepted"}}));
    expect_error(
        server.client().Post(base + "/api/v1/input", R"({"text":"missing user"})", "application/json"),
        400,
        "bad_request");
    expect_error(
    ...
    const auto agent = server.client().Post(base + "/api/v1/actions/default-agent", R"({"persona_id":"guide"})", "application/json");
    ASSERT_TRUE(agent);
...
TEST(SessionRoutes, SeparatesNonLivePageFromApiAndRejectsInvalidBodiesBeforeLookup) {
    ...
    SessionRegistry registry({.session_limit = 1}, [&starts](const SessionKey&, WakeNotifier&) {
        ++starts;
        return std::make_unique<RouteController>(std::make_shared<Calls>());
    });
    RouteServer server(registry, {.request_body_limit = 32, .prompt_limit = 8});
    ...
    expect_error(server.client().Post("/s/lobby/missing/api/v1/input", R"({"user":"u","text":"123456789"})", "application/json"), 413, "prompt_too_large");
...
TEST(SessionRoutes, RejectsDnsRebindingHostBeforeSnapshotsStreamsAndMutations) {
    ...
    expect_error(
        server.client().Post(
            base + "input", rebound, R"({"user":"reader","text":"rebound"})", "application/json"),
        403,
        "forbidden_host");
    {
        std::lock_guard lock(calls->mutex);
...
TEST(SessionRoutes, MapsAdmissionAndShutdownOutcomesWithoutExecutingRejectedCommands) {
    ...
    SessionRegistry stopping_registry({.session_limit = 1}, [stopping_calls](const SessionKey&, WakeNotifier&) {
        return std::make_unique<RouteController>(stopping_calls);
    });
...
TEST(SessionRoutes, MapsProcessShutdownDrainToServerStopping) {
    ...
    SessionRegistry registry(settings, [calls](const SessionKey&, WakeNotifier&) {
        return std::make_unique<RouteController>(calls);
    });
...
TEST(SessionRoutes, TimeoutAndClientDisconnectLeaveAcceptedCommandRunning) {
    ...
    expect_error(
        server.client().Post(path, R"({"user":"reader","text":"timeout"})", "application/json"),
        503,
        "command_timeout");
    {
        std::unique_lock lock(calls->mutex);
    ...
    SessionRegistry disconnect_registry(disconnect_settings, [disconnect_calls](const SessionKey&, WakeNotifier&) {
        return std::make_unique<RouteController>(disconnect_calls);
    });
