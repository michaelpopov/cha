Implement **Session 3** of selectable xAI Grok Voice Transcribe support in CHA.

Follow the Keep it simple rule in AGENTS.md. If a simpler design meets a requirement, use it.

Read [block1.md](block1.md), the shared contract in [block2.md](block2.md), and the commit bodies from [Session 2a](block2a.md) and [Session 2b](block2b.md) before starting. These are the durable handoff; previous chats are not required. Work on the branch containing those completed commits.
...
Sessions 1, 2a, and 2b should already have:
...
5. Remove temporary Session 1/2 scaffolding, TODOs, or duplicate abstractions.
...
- read the committed interface/file-location and test summaries from Sessions 1, 2a, and 2b,
...
- unexpected interim events are ignored; final words follow Session 2's single-cursor rule
- long dictation
- silence
- punctuation
...
Requirements:
- use Session 2's 19 completed-batch credits (including the one native in-flight batch) plus one reserved partial/final-flush batch,
...
Use Session 2a's runtime/curl evidence and Session 2b's macOS packaged capture evidence. Repeat only checks affected by fixes in this session. Complete the listed Windows checks on a Windows host if available; do not make every session rerun both platforms. These are acceptance checks for the actual package, not another transport feasibility investigation.
...
Reuse the passing bridge/native lifecycle and runtime smoke results from 2a/2b unless this session changes the relevant native code or package. Run the checks affected by each fix. Do not broaden into unrelated suites without a failure or changed dependency that warrants it.

Fix regressions caused by Sessions 1, 2a, and 2b.
...
Commit the fixes with subject `voice input: finish integration checks (session 3)` and a body containing the following summary. If no fixes are needed, do not manufacture a cleanup change or empty commit; add a concise validation record to this document and commit that. Repeat the summary in chat:
