...
| Binding | `0.0.0.0`, so another machine on the same network can open the application. No authentication and no transport security in v1. |
...
| Build ownership | npm builds and tests browser files; CMake builds C++; the packaging script assembles them. |
...
| `npm run build` | Type-check and produce production files in `webapp/dist/`. |
...
| `npm run stage -- --root <dir>` | Replace `<dir>/web/` with the production frontend build for local integration testing. |

Frontend dependencies are installed under `webapp/node_modules/`, not globally.
...
## Block 1 — Application root and listener address

**Goal.** `chaweb` finds its files relative to the application root rather than
the working directory, and can be reached from another machine on the local
network.
...
2. **Resolve the root in `web_main.cpp`.** Change `main()` to accept `argc` and
   `argv`, take a `--root <path>` override when present, and otherwise use
   `executable_directory()`.
3. **Pass the root everywhere the entry point currently implies the working
   directory** ([web_main.cpp](../src/apps/web_main.cpp#L24)):
   `load_dotenv(root / ".env")`, `load_application_config(root)`,
   and `Workspace(root, config)`. Use the returned `config.log_file` unchanged:
   `load_application_config(root)` already resolves a relative logging path
   beneath the root.
4. **Leave `console_main.cpp` and `tui_main.cpp` unchanged.** The terminal
   frontends are launched from inside a workspace and must stay
   working-directory relative. This block changes `chaweb` only.
5. **Update the process test helper.** `WebServerProcess` currently `chdir`s
   into the workspace before exec ([web_server_process.cpp](../tests/support/web_server_process.cpp#L136));
   it must pass `--root <workspace>` instead, because the child's working
   directory no longer determines anything.
6. **Accept a wildcard listener address.** When the configured host is `0.0.0.0`
...
7. **Leave the development workspace on `127.0.0.1`.** There is no reason to
   expose a development machine, and the wildcard path is covered by the tests
   below rather than by daily use. Block 7 ships `host = "0.0.0.0"` in the
   packaged `app.toml`.
...
- A relative `log_file` resolves under the root; an absolute one is untouched.
- `chaweb` started from an unrelated working directory serves `/health` and
  `/api/v1/bootstrap` from the workspace named by `--root`.
...
`chaweb --root <dir>` behaves identically from any working directory, and a
workspace configured with `0.0.0.0` answers a browser on a second machine.
...
6. **Set a Content Security Policy** on the shell response. Start strict:
   ...
   needs nothing here.
7. **Wire the root through `web_main.cpp`**: `AssetHandler(root / "web")`.
...
9. **Keep the repository runnable between Blocks 2 and 3.** Commit the same
   minimal `index.html` and `assets/app.js` under [`workspace/web/`](../workspace),
   the development workspace `chaweb` is normally started against. They are
   temporary development assets, not the eventual customer frontend; Block 3
   replaces them with a staged Vite build and stops tracking the directory.
   The C++ suite does not depend on them, because step 8 gives every test its
   own temporary web root.
...
A hand-written `web/index.html` and `web/assets/app.js` are served correctly at
`/`, at a deep link, and at `/assets/app.js`, with every safety case rejected.
...
2. **Create `webapp/`** with the layout in
   ...
   `components/`, `api/`, `state/`, `styles/`, and `main.tsx`.
...
7. **Build the static component tree** from
   ...
   Recent, and the gear; exclusive main area; chat composer with the context
   ...
   remains read-only.
8. **Add the staging command.** `npm run stage -- --root <dir>` first runs the
   production build, then safely replaces `<dir>/web/` with `webapp/dist/`.
   This is for local integration. The Block 7 packaging script performs the
   corresponding copy for the customer package; CMake never installs npm
   dependencies or copies browser source.
   Run it once as `npm run stage -- --root ../workspace` to replace the Block 2
   placeholders, then stop tracking `workspace/web/` in git and ignore it: from
   here on its contents are build output with hashed filenames, and leaving it
   tracked would make every build look like an edit. Document the one-time
   command, since a fresh checkout has no `workspace/web/index.html` and
   `chaweb` will say so at startup.
...
10. **Ignore generated directories**: `webapp/node_modules/`,
    `webapp/dist/`, `workspace/web/`, Playwright output, and local staged
    package directories.
...
4. **Decide the Settings entry point** and implement that decision. The design
   contract places a gear button in the sidebar; omitting it or making it
   informational amends that contract.
5. **Write one reproducible packaging command.** Its script runs `npm ci`,
   ...
   `webapp/dist/`, builds `chaweb`, creates a clean versioned output directory,
   and copies the native executable, prepared workspace, configuration, and
   browser build into it. It emits `.env.example`, never a `.env` containing a
   real key.
6. **Assemble the deployment directory** per
   [webapp.md](webapp.md#production-deployment-directory): executable,
   `app.toml`, `.env.example`, `web/`, the prepared workspace, and `logs/`.
   Treat it as one versioned unit and reject stale files from an earlier package
   assembly. The packaged `app.toml` sets `host = "0.0.0.0"`; a packaging check
   asserts that, since the value is what makes the application reachable and a
   silent revert to loopback would look like a network fault.
7. **Write the launcher** for the chosen primary OS: start `chaweb --root <the
   deployment directory>`, poll `/health` until ready, open
   `http://127.0.0.1:8080/`, and keep a visible window that stops the process.
   Bound the health wait, detect early process exit, and display the startup
   error instead of polling forever. Print the machine's LAN address alongside
   the local one, so the address to give someone else does not have to be
   worked out by hand, and state plainly in that window that anyone on the
   network who opens it can read and continue the conversations.
8. **Complete and run the existing Playwright suite** against the assembled
   package, covering the minimum flows in
   [webapp.md](webapp.md#testing-strategy), including interruption and rapid
   Recent navigation.
9. **Test on a clean machine** representative of the customer's laptop.
...
From [webapp.md](webapp.md#decisions-still-required), with the block each one
blocks:

| Decision | Needed before |
| --- | --- |
| Primary operating system | Block 7 launcher and clean-machine test, and the command that reports the LAN address |
| Workspace ownership: prepared or customer-edited | Block 7 |
| API-key setup: customer creates `.env` from `.env.example` using instructions | Block 7 |
| Settings: omit or informational page | Block 7 |
| Port conflicts: fixed 8080 with a clear error | Block 7 |

None blocks Blocks 1 through 6. The executable-directory utility implements all
three supported platform branches from the start, while the initial frontend
target is already fixed to current Chrome and Edge. The primary operating
system selects the first customer launcher and clean-machine environment.

## Out of scope

Attachments, editable Settings, and any mutation API for personas, characters,
or forums. Installers, code signing, background services, and automatic updates.
Expanding template directives inside `CHARACTER.md`, which is an accepted rough
edge.
