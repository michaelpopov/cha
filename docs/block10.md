...
Delete the internal application HTTP/SSE transport and obsolete deployment machinery after proven parity, then verify final packages, retained behavior, and measured net simplification.

This is the only block authorizing complete server/legacy frontend removal. Preserve outbound provider HTTP/SSE/R2 and necessary test servers, domain behavior, native harnesses, database format, and current supported desktop scope. Do not treat deleting tests or renaming files as simplification.

## Prerequisites to verify before editing

- Both assembled native desktop packages and their upgrade/verification paths
  pass without chaweb, development origins, or shipping test hooks. Actual native
  dialog/microphone/media/security/lifetime evidence is available for both hosts.
- Every retained operation and assertion has a verified replacement; the final
...
Plan for **200–300K tokens of working context** within a **400K-token window**,
...
10. Repeat the platform-feasibility work's measurements with the same file sets/method. Report net
...
- Account for every deletion against retained behavior/tests. Search production,
  packaging, configuration, and frontend code for listener/port/cookie/Host logic,
  EventSource, SSE socket/orphan machinery, API proxies, chaweb, and fallback flags;
...
- Complete the embedded real-host/race/upgrade checks for final media, maintenance,
  long conversations, Stop under load, renderer loss, retirement, and work-active
  quit. Reuse passing same-revision evidence; rerun affected checks after fixes.
- Repeat the initial measurement method/file sets. Report handwritten production,
  generated code, tests/harnesses, packaging, dependencies, and configuration
  separately; treat moves as moves and explain remaining necessary native costs.
- Resolve all inventory and handoff items or report them as blockers. Do not
  declare completion with a permanent HTTP fallback, missing platform evidence,
  or useful behavior tests removed merely to improve the line-count result.
...
Provide final implementation summary, measured before/after results,
