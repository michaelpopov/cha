# Block 7 — Providers, credentials, OAuth, and settings
...
**Initial status:** not started. **Environment:** a supported native compiler, Node/npm, macOS, and Windows/WebView2.

## Objective and scope

Migrate providers and provider tests, API keys, OAuth, R2 configuration, styles/voices, and non-secret voice runtime/settings with shared validation, lifecycle, and UI behavior.

Media settings belong here; voice capture/setup execution, synthesis, and audio jobs belong to the media stage. Preserve the existing provider/authentication protocols and UI interaction model. Keep temporary HTTP adapters until final cutover.

## Prerequisites to verify before editing

- Native Application/bridge/frontend, owners, actual host runners, bounded
  delivery, document lifetime, and shutdown are implemented and tested.
- Maintenance/context exclusion and vault/transfer/native-action operations pass
  common and both-host checks; R2 transfer operations can use configured test data.
- Session/workspace CRUD, canonical publication, affected-session invalidation,
  native export, shared type generation, and wire fixture checks are implemented.
- Native provider/credential/OAuth backends already exist; extract policy from
  routes rather than replacing those backends or moving secrets into JavaScript.
...
Plan for **180–280K tokens of working context** within a **400K-token window**,
...
Start with `src/web/{settings_routes,
...
4. Migrate R2 settings get/save/delete with existing references/configuration
   rules, then verify the transfer UI against the maintenance/vault/native-action implementation's operations.
...
7. Migrate voice-input/output settings and non-secret runtime capabilities. Native
   ...
   natively. Keep incomplete media execution explicitly unavailable until the media implementation stage.
...
- Test provider CRUD/configuration/usage constraints and deterministic provider
  tests with success, malformed config, failure, total/idle deadlines, and
  approved public errors; slow work must not starve session Stop.
- Assert key reads return metadata/presence only, write-only secret buffers are
  cleared, and normal replies/fixtures/logs contain no stored secret value.
- Verify OAuth status/start/poll/disconnect and approved external authorization
  links; old document/context completions cannot update a newer account/vault UI.
- Test R2 configuration get/save/delete and the already-implemented transfer UI
  with dedicated fixtures; isolate any real account/cloud verification.
- Test style/voice CRUD/defaults/reference restrictions, voice settings validation,
  non-secret runtime DTOs, immediate appearance changes, affected-session updates,
  and persistence through native reload/restart.
- Extend lifecycle/saturation/shutdown checks to provider tests and OAuth; preserve
  unknown-outcome semantics when a caller stops waiting. Run schema/fixture,
  TypeScript, focused UI, and actual native settings flows.
...
Provider, credential, OAuth, R2, appearance, and voice configuration
...
operations remain for the media implementation stage.
...
Provide bindings and tests, secret-boundary evidence, provider execution/
