Implement **Session 2a: native xAI transport and bridge** in CHA.
...
3. Register the four bridge methods and add DTO/client types and typed client calls. Audio and stop replies carry ordered `pieces: string[]`; they must preserve finalized-piece boundaries for Session 2b's command handling.
...
Keep the browser's xAI unavailable-transport stub. Do not implement AudioWorklet capture, command formatting, composer wiring, or packaged microphone checks in this session. Those belong to [Session 2b](block2b.md). Native timestamp deduplication is complete here; 2b does not duplicate it in TypeScript.
...
Complete this boundary and stop. Commit with subject `voice input: add native xAI streaming (session 2a)`. The commit body must list the concrete worker/normalizer files, all four bridge method signatures, typed-client entry points, test commands/results, and any unrun platform checks. Explicitly state that browser capture remains the intentional 2b stub. Commit only feature/prerequisite changes, preserving unrelated user work. Session 2b must be able to continue from this commit and the repository documents without this chat.
